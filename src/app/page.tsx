...
export default function LeaderboardPage() {
  ...
  const processed = useMemo(() => {
    ...
    if (filter === "contested") list = list.filter((u) => u.attendedContestsCount > 0);
  ...
  const batchStats = useMemo(() => {
    ...
    users.filter(u => !u.error).forEach(u => {
      const year = u.yearStudying || "Unknown";
      if (!batches[year]) {
        batches[year] = { year, totalStudents: 0, totalSolved: 0, avgSolved: 0, totalEasy: 0, totalMedium: 0, totalHard: 0 };
      }
  ...
  // Summary stats
  const totalSolvedAll = users.filter((u) => !u.error).reduce((a, u) => a + u.totalSolved, 0);
  const topRating = Math.max(...users.filter((u) => !u.error).map((u) => u.contestRating), 0);
  const avgSolved =
  ...
  const getRankDisplay = (rank: number) => {
    if (rank === 1) return <span className="rank-1 font-mono font-bold text-lg">🥇</span>;
    if (rank === 2) return <span className="rank-2 font-mono font-bold text-lg">🥈</span>;
    if (rank === 3) return <span className="rank-3 font-mono font-bold text-lg">🥉</span>;
    return (
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bu-dark)" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm pl-5" style={{ color: "var(--bu-sub)" }}>
                ...
                {users.filter((u) => !u.error).length} coders registered
                {lastRefreshed && (
        ...
        {qotw && (
          <div className="mb-8 rounded-2xl p-6 border relative overflow-hidden animate-fade-in group" style={{ background: "var(--bu-card)", borderColor: "var(--bu-red)" }}>
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl shadow-lg border" style={{ background: "var(--bu-dark)", borderColor: "var(--bu-border)", color: "var(--bu-red)" }}>
                  🔥
                </div>
                <div>
        ...
        <div className="flex justify-center mb-8 animate-fade-in">
          <div className="flex gap-2 p-1.5 rounded-2xl border" style={{ background: "rgba(255,255,255,0.03)", borderColor: "var(--bu-border)" }}>
            <button 
              ...
              style={{ background: viewMode === "individuals" ? "var(--bu-red)" : "transparent", color: viewMode === "individuals" ? "#fff" : "var(--bu-sub)" }}
            >
              👤 Individuals
            </button>
            <button 
              ...
              style={{ background: viewMode === "batches" ? "var(--bu-red)" : "transparent", color: viewMode === "batches" ? "#fff" : "var(--bu-sub)" }}
            >
              ⚔️ Batch Wars
            </button>
          </div>
          ...
          <div className="flex gap-2">
            {(["all", "top10", "contested"] as Filter[]).map((f) => (
              <button
                ...
                style={{
                  background: filter === f ? "var(--bu-red)" : "var(--bu-card)",
                  color: filter === f ? "#fff" : "var(--bu-sub)",
                  border: "1px solid",
                  borderColor: filter === f ? "var(--bu-red)" : "var(--bu-border)",
                }}
              >
                {f === "all" ? "All" : f === "top10" ? "Top 10" : "Contestants"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div
          ...
          style={{ borderColor: "var(--bu-border)", background: "var(--bu-card)" }}
        >
              ...
              <tbody>
                {loading ? (
                  ...
                  processed.map((user, i) => (
                    ...
                    >
                      {/* Rank */}
                      <td className="px-6 py-4 w-16">{getRankDisplay(i + 1)}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-3 font-mono text-sm">
                          ...
                          <span className="medium">{user.mediumSolved}</span>
                      ...
                      <td className="px-6 py-4">
                        {user.contestRating > 0 ? (
                          ...
                          <span style={{ color: "var(--bu-muted)" }}>—</span>
                        )}
                      </td>
          ...
          {!loading && processed.length > 0 && (
            ...
            >
              <span>
                Showing {processed.length} of {users.filter((u) => !u.error).length} coders
              </span>
              <span>
                Data via{" "}
                <a
        ...
        {users.some((u) => u.error) && (
          <p className="mt-4 text-xs text-center" style={{ color: "var(--bu-sub)" }}>
            {users.filter((u) => u.error).length} username(s) could not be fetched and are hidden.
          </p>
        )}
          </>
        ) : (
          ...
          <div className="animate-fade-in">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
              {batchStats.map((batch, idx) => (
                <div key={batch.year} className="relative rounded-2xl p-6 border overflow-hidden group transition-all hover:-translate-y-1 hover:shadow-xl" style={{ background: "var(--bu-card)", borderColor: idx === 0 ? "var(--bu-red)" : "var(--bu-border)" }}>
                  {idx === 0 && (
                  ...
                  <div className="flex justify-between items-start mb-4 relative z-10">
                    <div>
                      <h3 className="text-2xl font-bold text-white mb-1">{batch.year === "Unknown" ? "Unassigned" : `Batch ${batch.year}`}</h3>
                      <p className="text-sm" style={{ color: "var(--bu-sub)" }}>{batch.totalStudents} coders</p>
                    </div>
                    {idx === 0 && <span className="text-4xl filter drop-shadow-lg" title="Current Leader">👑</span>}
                    {idx === 1 && <span className="text-4xl filter drop-shadow-lg opacity-80">🥈</span>}
                    {idx === 2 && <span className="text-4xl filter drop-shadow-lg opacity-60">🥉</span>}
                  </div>

                  <div className="space-y-4 relative z-10">
                    <div className="p-4 rounded-xl border" style={{ background: "var(--bu-dark)", borderColor: "var(--bu-border)" }}>
                      <p className="text-sm font-medium mb-1" style={{ color: "var(--bu-sub)" }}>Average Solved</p>
                      <p className="text-3xl font-mono font-bold text-white">
                        {batch.avgSolved.toLocaleString()} <span className="text-sm font-sans font-normal opacity-50">/ coder</span>
                      </p>
                    </div>
                    
                    <div className="flex gap-2">
                      <div className="flex-1 p-3 rounded-xl border text-center" style={{ background: "rgba(255,255,255,0.02)", borderColor: "var(--bu-border)" }}>
                        <p className="text-xs mb-1 font-medium" style={{ color: "#00b8a3" }}>Easy</p>
                        <p className="font-mono text-sm text-white">{batch.totalEasy.toLocaleString()}</p>
                      </div>
                      <div className="flex-1 p-3 rounded-xl border text-center" style={{ background: "rgba(255,255,255,0.02)", borderColor: "var(--bu-border)" }}>
                        <p className="text-xs mb-1 font-medium" style={{ color: "#ffc01e" }}>Med</p>
                        <p className="font-mono text-sm text-white">{batch.totalMedium.toLocaleString()}</p>
                      </div>
                      <div className="flex-1 p-3 rounded-xl border text-center" style={{ background: "rgba(255,255,255,0.02)", borderColor: "var(--bu-border)" }}>
                        <p className="text-xs mb-1 font-medium" style={{ color: "#ff375f" }}>Hard</p>
                        <p className="font-mono text-sm text-white">{batch.totalHard.toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 h-1.5 w-full bg-black/40 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        ...
                        background: idx === 0 ? "linear-gradient(90deg, #ff6b6b, var(--bu-red))" : "var(--bu-sub)"
                      }}
                    />
                  </div>
                </div>
              ...
              {batchStats.length === 0 && (
                <div className="col-span-full py-20 text-center border rounded-2xl" style={{ borderColor: "var(--bu-border)", background: "var(--bu-card)" }}>
                  <p className="text-white font-medium">No batch data available yet.</p>
                </div>
              )}
            </div>
          </div>
